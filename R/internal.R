...
### It matches \code{nfi.nr}, province (\code{pr}), and species against
### \code{SNFI43_all_volume_coefficients}, then evaluates the requested
### output in \code{parametro} using the functions defined in
### \code{snfi_volume_equations.r}. The output is the input data frame
### plus only the requested volume column(s). When \code{parametro}
### includes \code{"VCC"} and the new method fails, the function uses
### the legacy \code{parEqVcc} route as an internal fallback.
(
    nfi,
    cub.met = "freq",
    parametro = c("VCC"),
    ...
    col_spec <- pick_col(c("especie", "codigo_especie"))
    ...
    valid_param <- c("VCC", "VSC", "IAVC", "VLE", "V")
    parametro <- intersect(parametro, valid_param)

    if (length(parametro) == 0L)
        stop("parametro must contain at least one of: ",
             paste(valid_param, collapse = ", "),
             call. = FALSE)
    ...
    for (param in setdiff(parametro, "V"))
        out[[tolower(param)]] <- NA_real_
    ...
    if (can_use_new) {
        ...
        match_coef_rows <- function(pr, especie) {
            ...
            x <- coef_tab[
                ...
                coef_tab$codigo_provincia == suppressWarnings(as.numeric(as.character(pr))),
                ,
                drop = FALSE
            ]
            ...
            if (!is.na(sp_num) && "codigo_especie" %in% names(x)) {
            ...
            if ("especie" %in% names(x)) {
                y <- x[tolower(x$especie) == tolower(as.character(especie)),
                       ,
                       drop = FALSE]
                if (nrow(y) > 0L)
                    return(y)
        ...
        for (i in seq_len(nrow(out))) {
            ...
            if (any(intersect(parametro, c("VSC", "IAVC", "VLE"))) > 0L) {
                pars_vcc <- pars_i[pars_i$parametro == "VCC", , drop = FALSE]
                if (nrow(pars_vcc) > 0L) {
                    pars_vcc <- pars_vcc[1L, , drop = FALSE]
                    req_vcc <- needed_for_param("VCC",
                                                modelo = as.numeric(pars_vcc$modelo[1]))
                    if (all(req_vcc %in% available_std)) {
                        fun_vcc <- get_fun_for_param("VCC")
                        if (!is.null(fun_vcc)) {
                            vcc_tmp[i] <- tryCatch(
                                as.numeric(fun_vcc(
                                    dbh_mm = dbh_mm,
            ...
            for (param in intersect(parametro, c("VCC", "VSC", "IAVC", "VLE"))) {
                ...
                req <- needed_for_param(param, modelo)
                basic_req <- setdiff(req, "vcc")

                if (!all(basic_req %in% available_std))
                    next
                ...
                vcc_dm3 <- if (!is.na(vcc_tmp[i])) vcc_tmp[i] * 1000 else NULL
                ...
                out[[tolower(param)]][i] <- as.numeric(val) / 1000
            }
        }
    ...
    need_legacy <- any(parametro %in% c("V", "VCC"))

    if (need_legacy) {

        if (!all(needed_legacy %in% tolower(names(nfi_orig))) || length(spec_legacy) == 0L) {
            warn_msg <- c(
                warn_msg,
            ...
            if (nrow(vt) > 0L) {
                ...
                if (length(spm) > 0L) {
                    ...
                    if (nrow(mmd) > 0L) {
                        ...
                        key_cols_out <- intersect(c(col_pr, col_d), names(out))
                        key_cols_vv  <- intersect(c(col_pr, col_d), names(vv))

                        idx <- match(
                            apply(out[, key_cols_out, drop = FALSE], 1, paste, collapse = "_"),
                            apply(vv[, key_cols_vv, drop = FALSE], 1, paste, collapse = "_")
                        )

                        v_legacy <- rep(NA_real_, nrow(out))
                        ok <- !is.na(idx)
                        v_legacy[ok] <- vv$v[idx[ok]]
...
vol_cols_all <- c("v", "vcc", "vsc", "iavc", "vle")
keep_cols <- intersect(tolower(parametro), vol_cols_all)
drop_cols <- setdiff(intersect(names(out), vol_cols_all), keep_cols)

if (length(drop_cols))
    out <- out[, !names(out) %in% drop_cols, drop = FALSE]
    
    out_cols <- intersect(tolower(parametro), names(out))
    attr(out, "units") <- c(
        attr(nfi_orig, "units"),
        setNames(rep("m3", length(out_cols)), out_cols)
    )
