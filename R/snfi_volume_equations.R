...
(dbh_mm, ##<< Tree diameter at breast height in millimetres.
 h_m, ##<< Tree height in metres.
 pars ##<< One-row coefficient data frame with the model code and coefficients.
) {
  ##details<<
  ##details<< This function evaluates the SNFI equation for 
  ##details<< merchantable volume over bark (VCC) and returns the result in dm3.
  ##details<< It currently supports models 1 and 11 from the SNFI coefficient table.
  ##value<< Numeric VCC in dm3. Returns `NA_real_` when `pars` is empty
  ##value<< or when the model is not recognised.
...
(vcc, ##<< Merchantable volume over bark in dm3.
 pars ##<< One-row coefficient data frame with the model code and coefficients.
  ...
  ##details<<
  ##details<< This function evaluates the SNFI equation for 
  ##details<< merchantable volume under bark (VSC) and returns the result in dm3.
  ##details<< It currently supports model 7.
  ##value<< Numeric VSC in dm3. Returns `NA_real_` when `pars` is empty
  ##value<< or when the model is not recognised.
...
(dbh_mm = NULL, ##<< Tree diameter at breast height in millimetres, when required by the model.
 ...
 pars ##<< One-row coefficient data frame with the model code and coefficients.
  ...
  ##value<< Numeric IAVC in dm3. Returns `NA_real_` when `pars` is empty,
  ##value<< when required inputs are missing, or when the model is not recognised.
...
(dbh_mm = NULL, ##<< Tree diameter at breast height in millimetres, when required by the model.
 ...
 pars ##<< One-row coefficient data frame with the model code and coefficients.
  ...
  ##value<< Numeric VLE in dm3. Returns `NA_real_` when `pars` is empty,
  ##value<< when required inputs are missing, or when the model is not recognised.
